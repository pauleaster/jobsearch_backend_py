# src\controllers\search_term_controller.py
...
@router.get("/{term_id}", response_model=SearchTermSchema)
...
@router.post("/", response_model=SearchTermSchema, status_code=status.HTTP_201_CREATED)
def create_search_term(term_dto: SearchTermSchema, db: Session = Depends(get_db)):
