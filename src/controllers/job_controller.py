# Jobsearch_backend_py\src\controllers\job_controller.py
...
def create_job(job_dto: JobDto, db: Session = Depends(get_db)):
    """Create a new job."""
    service = JobService(db)
...
def update_job(job_id: int, job_dto: JobDto, db: Session = Depends(get_db)):
...
def patch_job_field(job_id: int, patch_dto: JobPatchFieldDto, db: Session = Depends(get_db)):
